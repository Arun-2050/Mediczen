...
CREATE TABLE IF NOT EXISTS patients (
  ...
  status TEXT CHECK (
    status IN ('Active', 'Recovered', 'Critical', 'Discharged')
  ) DEFAULT 'Active',
  phone TEXT,
  email TEXT,
...
CREATE TABLE IF NOT EXISTS patient_records (
  ...
  vitals JSONB,
  -- e.g. {"bp": "120/80", "pulse": 72, "temp": "98.6F", "spo2": "99%"}
  notes TEXT,
  ai_summary TEXT,
...
CREATE TABLE IF NOT EXISTS invoices (
  ...
  -- e.g. [{"description": "Consultation Fee", "qty": 1, "price": 150}]
...
CREATE POLICY "Doctors can access their own profile" ON doctors FOR ALL USING (id = auth.uid()) WITH CHECK (id = auth.uid());
CREATE POLICY "Doctors can access their own patients" ON patients FOR ALL USING (doctor_id = auth.uid()) WITH CHECK (doctor_id = auth.uid());
CREATE POLICY "Doctors can access their own records" ON patient_records FOR ALL USING (doctor_id = auth.uid()) WITH CHECK (doctor_id = auth.uid());
CREATE POLICY "Doctors can access their own appointments" ON appointments FOR ALL USING (doctor_id = auth.uid()) WITH CHECK (doctor_id = auth.uid());
CREATE POLICY "Doctors can access their own schedules" ON schedules FOR ALL USING (doctor_id = auth.uid()) WITH CHECK (doctor_id = auth.uid());
CREATE POLICY "Doctors can access their own invoices" ON invoices FOR ALL USING (doctor_id = auth.uid()) WITH CHECK (doctor_id = auth.uid());
CREATE POLICY "Doctors can manage their own portal links" ON patient_links FOR ALL USING (
  EXISTS (
    SELECT 1
    FROM patients
    WHERE patients.id = patient_links.patient_id
